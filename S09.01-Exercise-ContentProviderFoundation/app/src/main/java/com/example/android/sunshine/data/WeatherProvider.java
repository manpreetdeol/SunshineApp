...
//      TODO (9) Handle queries on both the weather and weather with date URI
        ...
        switch (match) {
            case CODE_WEATHER:
                cursor = sqLiteDatabase.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
...
//                String uriDate = uri.getPathSegments().get(1);
                String uriDate = uri.getLastPathSegment();

                String mSelection = WeatherContract.WeatherEntry.COLUMN_DATE + "=?";
                String[] mSelectionArgs = new String[] {uriDate};
